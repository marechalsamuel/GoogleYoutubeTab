...
const TABS_CLASS_NAMES_V1 = ['hdtb-mitem']
...
const createTabV1 = ({ label, url, icon, customOptions = {} }) => { // TODO use cloneNode
  ...
  if (!tab) {
    ...
    const brother = document.querySelectorAll(TABS_CLASS_NAMES_V1.map(tabsClassName => `.${tabsClassName}`).join(''))[1]
    brother.parentNode.insertBefore(container, brother)
  }
}

const createIconV1 = (pathD) => {
  ...
  icon.className = ICONS_CLASS_NAME_V1
...
const main = () => {
  ...
  if (intentsLeft > 0)
    return window.setTimeout(main, 500)
}
